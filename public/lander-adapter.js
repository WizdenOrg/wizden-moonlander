...
// Typed-decision request building, answer parsing and provider transport for the lander.
// Shared by the browser (tech spec) and the Node relay, tests and evaluation.
(function (root, factory) {
  if (typeof module === 'object' && module.exports) module.exports = factory(require('./lander-physics'), require('./lander-guidance'));
  else root.LanderAdapter = factory(root.LanderPhysics, root.LanderGuidance);
}(typeof self !== 'undefined' ? self : this, function (physics, guidance) {
  ...
  async function decide(telemetry, connection = {}, options = {}) {
    ...
    const outputs = await Promise.all(bodies.map((body) => post(provider, url, connection.token, body, connection.timeoutMs)));
  ...
  async function post(provider, url, token, body, timeoutMs = 30000) {
    let response;
    try {
      response = await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json', ...provider.headers(token) }, signal: AbortSignal.timeout(timeoutMs), body: JSON.stringify(body) });
    } catch (error) {
      if (error.name === 'TimeoutError') throw new Error(`${provider.label} endpoint timed out. ${provider.unreachable}`);
      throw new Error(`${provider.label} endpoint could not be reached. ${provider.unreachable}`);
    }
    if (!response.ok) {
  ...
  return { decide, PROMPT_MODES, factSentences, THRESHOLDS, cleanBriefing, MAX_BRIEFING_LENGTH, noulScore, PROVIDERS, JEV_URL, buildRequest, buildRequests, choiceFrom, controls, VARIANTS, DEFAULT_VARIANT, CRITERIA };
}));
